...
export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    const { nombre, proveedor, url, fecha, usuario } = req.body;
    if (!nombre || !proveedor || !url) {
      return res.status(400).json({ error: "Datos incompletos" });
    }
    const products = (await redis.get(KEY)) || [];
    const id = Date.now().toString();
    products.push({ id, nombre, proveedor, url, fecha: fecha || "" });
  ...
  if (req.method === "PUT") {
    const { id, nombre, proveedor, url, fecha, usuario } = req.body;
    ...
    products[idx] = { id, nombre, proveedor, url, fecha: fecha || "" };
    await redis.set(KEY, products);
